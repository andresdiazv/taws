...
#define TOL_UNIT_CONVERSION 1e-9
...
{
    TEST_ASSERT_DOUBLE_WITHIN(TOL_UNIT_CONVERSION, 0.0, feet_to_meters(0.0));
}

/* 1 ft = 0.3048 m */
void test_feet_to_meters_exact_definition_one_foot(void)
{
    TEST_ASSERT_DOUBLE_WITHIN(TOL_UNIT_CONVERSION, 0.3048, feet_to_meters(1.0));
}

/* 500 ft is the TAWS height callout */
void test_feet_to_meters_callout_height_500ft(void)
{
    TEST_ASSERT_DOUBLE_WITHIN(TOL_UNIT_CONVERSION, 152.4, feet_to_meters(500.0));
}
...
{
    TEST_ASSERT_DOUBLE_WITHIN(TOL_UNIT_CONVERSION, 0.0, meters_to_feet(0.0));
}

/* 1 / 0.3048 */
void test_meters_to_feet_one_meter(void)
{
    TEST_ASSERT_DOUBLE_WITHIN(TOL_UNIT_CONVERSION, 3.280839895013123, meters_to_feet(1.0));
}

void test_meters_to_feet_large_value(void)
{
    TEST_ASSERT_DOUBLE_WITHIN(TOL_UNIT_CONVERSION, 1640.4199475065617, meters_to_feet(500.0));
}
...
{
    TEST_ASSERT_DOUBLE_WITHIN(TOL_UNIT_CONVERSION, 0.0, meters_per_second_to_knots(0.0));
}

/* 3600 s/h divided by 1852 m/NM */
void test_mps_to_knots_one_mps(void)
{
    TEST_ASSERT_DOUBLE_WITHIN(TOL_UNIT_CONVERSION, 1.9438444924406047, meters_per_second_to_knots(1.0));
}

void test_mps_to_knots_large_value(void)
{
    TEST_ASSERT_DOUBLE_WITHIN(TOL_UNIT_CONVERSION, 971.9222462203023, meters_per_second_to_knots(500.0));
}
...
{
    TEST_ASSERT_DOUBLE_WITHIN(TOL_UNIT_CONVERSION, 1234.5, meters_to_feet(feet_to_meters(1234.5)));
}
